...
| 2026-09-18 | **CI 복구와 보안 점검** — 31회 연속 실패하던 CI를 고치고(8.3 단축 경로·Python 3.11 junction 탐지·드라이브 간 경로) 3분할 병렬화와 실행시간 기반 재배치로 41분 → 약 16분. 공개 검색의 LIKE 와일드카드를 리터럴로 처리하고 SQL 조립 803곳을 전수 점검해 인젝션 경로가 없음을 확인했습니다. |
