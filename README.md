...
HRMCP는 `HR Analytics`처럼 NCS에 동일 명칭의 분류·능력단위가 없는 역할을 공식 NCS 직무로
생성하지 않습니다. 현재 공개 MCP도 이 명칭을 한 번의 호출로 여러 NCS 범위로 자동 분해하거나,
가상 직무 프로파일을 DB에 저장하는 전용 기능은 제공하지 않습니다.

대신 ChatGPT·Claude 같은 외부 에이전트나 HR 담당자가 `인사기획`, `인사평가`, `통계조사`,
`빅데이터분석`, `데이터 시각화`처럼 실제 NCS 범위를 각각 조회한 뒤 능력단위·수행준거·대표 KSA
개념·훈련과정 근거를 결합하여 **탐색적 신직무 프로파일 초안**을 만들 수 있습니다. 동일한 대표
개념(`concept_id`)을 기준으로 중복 표현을 줄일 수 있지만, 포함 범위·역량 중요도·역할 수준·조직
고유 역량은 HR 담당자가 확정해야 합니다.

![NCS에 없는 HR Analytics 신직무를 관련 NCS 근거와 온톨로지 관계로 탐색적으로 설계하는 흐름](docs/images/hrmcp_new_job_ontology.png)

위 그림은 단일 MCP 호출이 신직무를 자동 생성한다는 의미가 아닙니다. 여러 NCS 조회 결과를 외부
에이전트와 HR 담당자가 조합하고 검토하는 활용 흐름이며, 근거는 다음처럼 구분합니다.

- **NCS 직접 근거**: 실제 능력단위, 능력단위요소, 수행준거, KSA, 훈련과정
- **온톨로지 기반 연결**: 저장된 개념·별칭·관계를 따라 찾은 인접 근거
- **모델·조직 제안**: 역할 구조, 중요도, NCS 밖의 추가 역량

이 접근은 신직무 초안 외에도 다음 작업으로 확장할 수 있습니다.

- **융합 직무 설계**: AI HR, People Analytics, HRBP, AX 기획처럼 여러 NCS 영역에 걸친 역할의
  과업과 역량 후보를 정리합니다.
- **기존 직무 재설계**: 새로 추가된 업무와 기존 직무의 공통·부족 KSA를 비교해 책임 범위를
  재구성합니다.
- **직무군·내부 이동 분석**: 직무 간 공통 KSA와 역량 갭을 근거로 인접 직무, 업스킬링,
  리스킬링 후보를 검토합니다.
- **평가체계 연결**: 직무기술서의 수행준거와 KSA를 구조화 면접 질문·평가요소·행동지표로
  연결합니다.
- **교육체계 설계**: 부족 KSA를 훈련목표·수준·시간·방법·시설 근거와 연결해 과정 묶음을
  검토합니다.

조직별 신직무 프로파일의 저장·버전관리, 사용자 정의 중요도 가중치, 직원별 스킬 매트릭스와
승계계획은 현재 공개 MCP의 완성 기능이 아니라 추가 제품화 영역입니다. 따라서 생성 결과는
직무기술서·역량모델·교육체계 설계를 위한 검토용 초안이며, 공식 NCS 정의·자격 인정·채용 판정이
아닙니다.
