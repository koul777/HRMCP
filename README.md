...
| 날짜 | 내용 |
| --- | --- |
| 2026-09-20 | **검토 품질 신호 강화** — 정의가 있는 온톨로지 개념 413,143건이 모두 템플릿형 boilerplate임을 품질 게이트에 명시하고, 이를 실질 정의나 사람 검토 완료로 간주하지 않도록 했습니다. 교육과정의 `required` 표시는 “필수 검토 후보”로 명확히 했으며, 품질·릴리스 요청과 실제 교육체계 설계 요청의 라우팅을 분리했습니다. |
| 2026-09-18 | **검색 품질 개선 기반 완성** — 실패 원인을 어휘 랭킹의 구조적 한계로 규명하고, 임베딩 모델 4종 비교 후 한국어 소형 모델(112MB, CPU 질의 4ms)로 의미 기반 구제 재랭킹을 구현했습니다. 개발 90문항 Hit@3 `0.800 → 0.833`, 회귀 세트 `0.875` 무변동. 공급자를 설정하지 않으면 기존 동작과 완전히 동일한 기본 비활성 상태입니다. |
| 2026-09-18 | **평가 체계 정비** — 개발 세트를 90문항(HR 45 + 비HR 통제 45)으로 만들고, 기록된 기준선보다 지표가 떨어지면 릴리스 전에 실패하는 회귀 게이트를 추가했습니다. |
| 2026-09-18 | **CI 복구와 보안 점검** — 31회 연속 실패하던 CI를 고치고(8.3 단축 경로·Python 3.11 junction 탐지·드라이브 간 경로) 3분할 병렬화로 41분 → 20분. 공개 검색의 LIKE 와일드카드를 리터럴로 처리하고 SQL 조립 803곳을 전수 점검해 인젝션 경로가 없음을 확인했습니다. |
...
- **2026-09-20 검토 품질 게이트·라우팅 안전성 보강**: `ontology_concepts.definition`이 채워진 413,143건 전부가 생성 템플릿형 boilerplate이고 실질 정의는 0건이라는 상태를 `review_debt:boilerplate_definition_ratio` 경고로 노출했습니다. 특정 승격 출처만으로 실질 정의를 boilerplate로 오분류하지 않도록 텍스트·템플릿 출처 규칙을 분리했으며, 사람의 명시적 결정 없이 `human_reviewed`·`accepted`·`reviewed`를 쓰지 않는 정책은 유지합니다. 추천 결과의 `required` 라벨은 자동 확정처럼 보이지 않도록 “필수 검토 후보”로 바꾸고, 직접 과업/KSA·훈련목표 근거가 없는 과정은 기존처럼 `supporting`으로 강등합니다. “품질 게이트·릴리스 준비도 보완” 요청은 운영 검토 표면으로, 릴리스 문맥이 포함돼도 명시적으로 교육체계 설계를 요청한 질의는 `plan_ncs_education_path`로 라우팅하도록 회귀를 보강했습니다. 전체 단위 테스트 2,624건(4건 skip), 최종 품질·라우터 테스트 96건, lint·smoke·ontology validate를 통과했으며 DB/API/검토 상태는 변경하지 않았습니다.
