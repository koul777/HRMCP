...
def _seedpack_item(seedpack_id: str, sequence: int, item: dict[str, Any]) -> dict[str, Any]:
    ...
    snapshot_payload = {
        ...
        "current_review_status": _current_review_status(issue, context),
        "context": context,
    }
    return {
        ...
        "source_context_excerpt": _context_excerpt(context),
        "issue": issue,
        "context": context,
    }
    ...
    *,
    ...
    item_records = [
        _transition_seedpack_item(
            ...
            index,
            scenario,
            cases_by_name.get(scenario.get("scenario_name"), {}),
        )
    ...
    batch_record = {
        ...
        "notes": [
            ...
            "Approve only scenarios whose scope matches and expected courses are acceptable.",
            "Do not treat candidate or candidate_auto scenarios as trusted readiness without human review.",
        ],
    }
    out_path.parent.mkdir(parents=True, exist_ok=True)
    with out_path.open("w", encoding="utf-8", newline="\n") as handle:
...
def write_review_seedpack_markdown(summary: dict[str, Any], seedpack_path: Path, out_path: Path) -> None:
    lines = [
        ...
        "- Preserve raw source fields; use review/status fields or approved derived fields only.",
    ]
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_transition_scenario_seedpack_markdown(
    ...
    lines = [
        ...
        "- Use `approve` only when current/target scope and expected courses are acceptable.",
        "- Candidate scenarios are not trusted readiness until a human reviewer approves them.",
        "- Preserve the original scenario row; use review/status fields in a later apply step.",
    ]
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text("\n".join(lines) + "\n", encoding="utf-8")
