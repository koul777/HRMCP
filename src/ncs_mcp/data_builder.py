...
def api_failure_message(evidence: dict) -> str:
    phase_names = {"source_invariant_check": "원본 DB 검사", "working_copy_backup": "작업 DB 복사",
                   "working_copy_invariant_check": "작업 DB 검사", "api_collection": "API 수집",
                   "post_collection_invariant_check": "수집 후 데이터 검사", "training_link_build": "교육과정 연결",
                   "final_invariant_check": "최종 원본 보존 검사"}
    reason_names = {"sqlite_dbstat_module_unavailable": "SQLite 진단 모듈(dbstat) 호환성 오류",
    ...
    if evidence.get("failed_phase"):
        details.append(phase_names.get(evidence["failed_phase"], evidence["failed_phase"]))
    if evidence.get("failure_reason"):
        details.append(reason_names.get(evidence["failure_reason"], evidence["failure_reason"]))
    ...
    for source, rows in (evidence.get("source_results") or {}).items():
        ...
        if failed:
            examples = ", ".join(f"{row.get('major_code')}:{row.get('error_type', '응답 완결성 미확인')}" for row in failed[:3])
    ...
    return "API 갱신 중단: " + " / ".join(details or [evidence.get("outcome", "원인 미기록")])
...
def inspect_workbook(path: Path) -> dict:
    ...
    try:
        for sheet in workbook.worksheets:
            ...
            try:
                ...
                raise BuilderError(f"시트 '{sheet.title}'의 필수 NCS 열이 맞지 않습니다: {exc}") from exc
            first = next(rows, None)
            sheets.append({"name": sheet.title, "estimated_rows": max(0, (sheet.max_row or 1) - 1),
    ...
    return {"filename": path.name, "bytes": path.stat().st_size, "sheets": sheets,
            "note": "행 수는 Excel 메타데이터 추정치입니다. 실제 건수는 전처리 후 확정됩니다."}


class DataBuilder:
    def __init__(self, root: Path = PROJECT_ROOT, progress: Callable[[str], None] | None = None):
    ...
    def exclusive(self):
        ...
        try:
            ...
            raise BuilderError("다른 Builder 작업이 실행 중입니다. 비정상 종료했다면 실행 프로세스 확인 후 operation.lock을 정리하세요.") from exc
        try:
    ...
    def versions(self) -> list[dict]:
        result = []
        for path in sorted((self.state / "versions").glob("*/build.json"), reverse=True):
            try:
    ...
    def _new(self, kind: str) -> tuple[Path, dict]:
        version = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S") + "_" + uuid.uuid4().hex[:8]
        folder = self._version_dir(version)
        folder.mkdir(parents=True)
        report = {"schema": "ncs_data_builder_version_v1", "version": version, "kind": kind,
    ...
    def build_delta(self, source: Path, baseline: Path) -> dict:
        ...
        with self.exclusive():
            ...
            self._space(baseline.stat().st_size * 2 + max(1024**3, preview["bytes"] * 10))
            folder, report = self._new("excel-delta")
            try:
                original = folder / "source.xlsx"
                self.progress("새 Excel 원본 보관 및 능력단위 변경분 비교")
                shutil.copyfile(source, original)
                report.update(source=preview, source_sha256=file_sha256(original), parent_database=str(baseline))
                delta = build_excel_delta(original, baseline, folder / "ncs.db", folder / "delta", self.progress)
                atomic_json(folder / "delta.json", delta)
                if not delta.get("ok"):
                    raise BuilderError("변경분 검증에 실패했습니다. delta.json을 확인하세요.")
    ...
    def package(self, version: str, deploy_root: Path) -> dict:
        ...
        with self.exclusive():
            ...
            report = build_release(self._version_dir(version), repo_root=self.root,
                                   deploy_root=deploy_root, expected_source_sha256=file_sha256(db),
                                   progress=self.progress)
            if not report.get("ok"):
                raise BuilderError("경량 패키지 생성·검증에 실패했습니다. release.json을 확인하세요.")
    ...
    def deploy(self, version: str, deploy_root: Path, production_url: str) -> dict:
        ...
        with self.exclusive():
            ...
            project = json.loads((Path(deploy_root) / ".vercel/project.json").read_text(encoding="utf-8"))
            bound_project = release.get("project") or release.get("project_configuration") or {}
            if bound_project.get("projectId") != project.get("projectId"):
                raise BuilderError("패키지를 만든 프로젝트와 선택 프로젝트가 다릅니다.")
            report = deploy_release(folder, production_mcp_url=production_url, progress=self.progress)
            if not report.get("ok"):
                raise BuilderError("Vercel 갱신 검증을 완료하지 못했습니다. release.json의 배포 상태를 확인하세요.")
            atomic_json(self.state / "deployed.json", {"version": version, "production_url": production_url,
    ...
    def refresh_api(self, source: Path, sources: list[str]) -> dict:
        with self.exclusive():
            ...
            try:
                ...
                if source.is_relative_to(self.state / "versions") and parent_report.exists():
    ...
    def copy_current(self) -> dict:
        with self.exclusive():
            ...
            try:
                self.progress("현재 MCP DB의 일관된 복사본 생성 (기존 API·검토 데이터 포함)")
