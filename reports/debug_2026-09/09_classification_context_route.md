...
|---|---|---|
| 업무용 차량 배차 관리 | 업무용 동산관리, 차량손해사정 기획 관리, 차량손해액 산정 | 차량운영관리, 업무용 동산관리, 업무지원 |
...
이번 범위 IDF 변경은 분류 필터가 없는 호출에는 적용되지 않는다. 동일
holdout 51건을 무필터로 재실행한 현재 기준은 Hit@3 `0.4902`이며, 이 수치를
과거의 다른 DB/스냅샷에서 계산된 `0.5490`과 섞어 비교하지 않는다.

## 추가 측정: 명시적 `major_code=02` 범위의 holdout 영향

holdout 파일은 변경하지 않고, 모든 케이스에 호출자 제공 필터
`{"major_code":"02"}`를 별도로 적용해 비교했다. 이 실험은 자동 major 추론이
아니며, UI/agent가 이미 직무 범위를 알고 전달하는 경우의 상한에 가까운
진단이다.

| 조건 | Hit@1 | Hit@3 | MRR |
|---|---:|---:|---:|
| 무필터 canonical DB | 0.3725 (19/51) | 0.4902 (25/51) | 0.4670 |
| 명시적 `major_code=02` | 0.5294 (27/51) | 0.6078 (31/51) | 0.5838 |

Hit@3가 `+0.1176p` 개선되지만 0.7 게이트에는 아직 미달한다. 따라서 다음
작업은 alias를 늘리는 것이 아니라 (1) 호출자 문맥을 실제 planner/UI까지
연결하고, (2) 형태소·경계 정규화를 별도 후보 tier로 실험하고, (3) 과업/KSA
근거를 이용한 재랭킹의 효과를 독립 dev 세트에서 측정하는 순서가 맞다.
