...
AUTOMATED_REVIEWER_IDS = {"", "automated_eval_gate", "automation", "mcp", "ncs_learning_mvp", "system"}
DERIVED_PLAN_LINK_STATUSES = REVIEWED_STATUSES | {"auto_candidate", "auto_linked", "candidate"}
...
def _trusted_review_provenance_blockers(
    *,
    reviewer_id: str,
...
def _audit_source_decision_packet(source_decision_packet: str | None) -> str | None:
    return normalize_source_decision_packet_ref(
        source_decision_packet,
        extensions=TRUSTED_REVIEW_PACKET_EXTENSIONS,
    )
...
def build_report_training_courses(
    ...
    *,
    ...
    reviewer_id: str = "ncs_reference_report_builder",
    notes: str = "",
    ...
    if review_status not in DERIVED_PLAN_LINK_STATUSES:
        return {
            "ok": False,
            "error": {
                "code": "unsupported_review_status",
                "allowed": sorted(DERIVED_PLAN_LINK_STATUSES),
            },
        }
    trusted_status = review_status in REVIEWED_STATUSES
    if trusted_status:
        provenance_blockers = _trusted_review_provenance_blockers(
        ...
        if provenance_blockers:
            return {
                "ok": False,
                "error": {
                    "code": "trusted_review_provenance_required",
    ...
    for module_seq, course in sorted(courses_by_seq.items()):
        ...
        for concept in concepts:
            ...
            item["count"] += 1
            previous_concept_link = conn.execute(
                """
                SELECT link_id, review_status
                FROM learning_module_concept_links
                WHERE learn_module_seq = ?
                  AND concept_id = ?
                  AND link_method = 'reference_report_training_course_concept'
                """,
                (module_seq, concept_id),
            ).fetchone()
            conn.execute(
                """
            ...
            if trusted_status and concept_link is not None:
                conn.execute(
                    """
                    ...
                    ) VALUES ('learning_module_concept_link', ?, 'build_report_training_courses', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        str(concept_link["link_id"]),
                        previous_concept_link["review_status"] if previous_concept_link is not None else None,
                        review_status,
                        reviewer_id,
                        notes,
                        _audit_source_decision_packet(source_decision_packet),
                        source_artifact_hash,
                        rationale or notes,
                        json.dumps(evidence_refs or [], ensure_ascii=False),
                        "ncs_reference.build_report_training_courses",
                        run_artifact,
                        timestamp,
                    ),
                )
    ...
    return {
        ...
        "note": (
            "REPORT-TRAINING rows are report-derived education candidates, not official NCS study-module API rows; trusted status was packet-backed."
            if trusted_status
            else "REPORT-TRAINING rows are report-derived education candidates, not official NCS study-module API rows; generated rows remain candidate/auto-linked until human review."
        ),
    }
    ...
    *,
    ...
    if review_status not in DERIVED_PLAN_LINK_STATUSES:
        return {
            ...
            "error": {
                "code": "unsupported_review_status",
                "message": "Derived plans must be created with a candidate or packet-backed trusted review status.",
                "allowed": sorted(DERIVED_PLAN_LINK_STATUSES),
            },
        }
    trusted_status = review_status in REVIEWED_STATUSES
    ...
    for unit in units:
        ...
        previous_unit_link = conn.execute(
            """
            SELECT link_id, review_status
            FROM learning_module_unit_links
            WHERE learn_module_seq = ?
              AND unit_code = ?
              AND link_method = 'ncs_derived_unit_plan'
            """,
            (module_seq, unit_code),
        ).fetchone()
        conn.execute(
            """
            INSERT INTO learning_module_unit_links(
        ...
        if trusted_status and unit_link is not None:
            conn.execute(
                """
                INSERT INTO review_audit_log(
                    entity_type, entity_id, action, previous_status, new_status,
                    reviewer_id, notes, source_decision_packet, source_artifact_hash,
                    rationale, evidence_refs_json, created_by_tool, run_artifact, created_at
                ) VALUES ('learning_module_ncs_link', ?, 'build_ncs_derived_learning_plans', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                """,
                (
                    str(unit_link["link_id"]),
                    previous_unit_link["review_status"] if previous_unit_link is not None else None,
                    review_status,
                    reviewer_id,
                    notes,
                    _audit_source_decision_packet(source_decision_packet),
                    ...
                    rationale or notes,
                    json.dumps(evidence_refs or [], ensure_ascii=False),
                    "ncs_reference.build_ncs_derived_learning_plans",
                    run_artifact,
                    timestamp,
                ),
            )
        ...
        for concept in chain["concepts"]:
            ...
            previous_concept_link = conn.execute(
                ...
                WHERE learn_module_seq = ?
                  AND concept_id = ?
                  AND link_method = 'ncs_derived_unit_concept'
                """,
                (module_seq, concept_id),
            ).fetchone()
            conn.execute(
            ...
            if trusted_status and concept_link is not None:
                conn.execute(
                    ...
                    INSERT INTO review_audit_log(
                        ...
                        reviewer_id, notes, source_decision_packet, source_artifact_hash,
                        rationale, evidence_refs_json, created_by_tool, run_artifact, created_at
                    ) VALUES ('learning_module_concept_link', ?, 'build_ncs_derived_learning_plans', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        str(concept_link["link_id"]),
                        previous_concept_link["review_status"] if previous_concept_link is not None else None,
    ...
    return {
        ...
        "trusted_status": trusted_status,
        "note": (
            "Derived plans are not official NCS study-module API rows; trusted status was packet-backed."
            if trusted_status
            else "Derived plans are not official NCS study-module API rows; generated rows remain candidate/auto-linked until human review."
        ),
    }
...
def review_exact_learning_module_name_links(
    ...
    *,
    ...
    trusted_review_status = "reviewed"
    provenance_blockers = _trusted_review_provenance_blockers(
        reviewer_id=reviewer_id,
        source_decision_packet=source_decision_packet,
        source_artifact_hash=source_artifact_hash,
    ...
    if provenance_blockers:
        return {
            ...
            "error": {
                "code": "trusted_review_provenance_required",
                "review_status": trusted_review_status,
                "blockers": provenance_blockers,
            },
            "trusted_review_provenance_required": True,
            "status_update_allowed_without_packet": False,
        }
    clauses, params = _classification_filter_clauses(
        "c",
        major_code=major_code,
    ...
    for row in rows:
        ...
        conn.execute(
            ...
            (
                ...
                max(0.95, float(row["confidence_score"] or 0)),
                timestamp,
                row["link_id"],
            ),
        )
        conn.execute(
            ...
            INSERT INTO review_audit_log(
                ...
                reviewer_id, notes, source_decision_packet, source_artifact_hash,
                rationale, evidence_refs_json, created_by_tool, run_artifact, created_at
            ) VALUES ('learning_module_ncs_link', ?, 'review_exact_learning_module_name_links', ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
            """,
            (
                str(row["link_id"]),
                row["review_status"],
    ...
    return {
        ...
        "applied_review_status": trusted_review_status,
        "trusted_review_provenance_required": True,
        "status_update_allowed_without_packet": False,
        "note": "Only packet-backed human decisions can mark exact module-name/unit-name links reviewed; classification-only links remain candidates.",
    }
    ...
    *,
    ...
    conn.execute(
        ...
        (
            ...
            rationale or notes,
            json.dumps(evidence_refs or [], ensure_ascii=False),
            "ncs_reference.review_learning_module_ncs_link",
            run_artifact,
            timestamp,
        ),
    )
