...
def qualification_retry_hygiene_report(
    ...
    *,
    ...
    attempted_unit_count = sum(int(row.get("unit_count") or 0) for row in status_counts)
    unattempted_unit_count = max(total_unit_count - attempted_unit_count, 0)
    target_attempted_unit_count = (total_unit_count * 9 + 9) // 10 if total_unit_count else 0
    additional_attempted_units_needed = max(target_attempted_unit_count - attempted_unit_count, 0)
    major_coverage_gaps = rows_to_dicts(
        conn.execute(
            """
            SELECT
                c.major_code,
                c.major_name,
                COUNT(cu.unit_code) AS total_unit_count,
                COUNT(status.unit_code) AS attempted_unit_count,
                SUM(CASE WHEN status.collection_status = 'error' THEN 1 ELSE 0 END) AS error_unit_count
            FROM competency_units cu
            LEFT JOIN ncs_qualification_collection_status status
                ON status.unit_code = cu.unit_code
            LEFT JOIN classifications c
                ON c.classification_id = cu.classification_id
            GROUP BY c.major_code, c.major_name
            ORDER BY (COUNT(cu.unit_code) - COUNT(status.unit_code)) DESC,
                     error_unit_count DESC,
                     c.major_code
            LIMIT ?
            """,
            (max_rows,),
    ...
    for row in major_coverage_gaps:
        total = int(row.get("total_unit_count") or 0)
        attempted = int(row.get("attempted_unit_count") or 0)
        row["unattempted_unit_count"] = max(total - attempted, 0)
        row["coverage"] = round(attempted / total, 4) if total else 0.0
    error_rows = rows_to_dicts(
        conn.execute(
            """
            SELECT
    ...
    return {
        ...
        "mode": "dry_run",
        "coverage_gap": {
            "release_target_ratio": release_target_ratio,
            "total_unit_count": total_unit_count,
            "attempted_unit_count": attempted_unit_count,
        ...
        "status_counts": status_counts,
        "major_coverage_gaps": major_coverage_gaps,
        "error_unit_count": error_count,
        "retry_candidate_unit_count": retry_ready_count,
        "retry_ready_unit_count": retry_ready_count,
        "retry_waiting_unit_count": max(0, error_count - retry_ready_count),
...
def write_qualification_retry_hygiene_markdown(report: dict[str, Any], out_path: Path) -> None:
    def append_api_guard(lines: list[str], payload: dict[str, Any]) -> None:
        guard = payload.get("api_execution_guard") if isinstance(payload.get("api_execution_guard"), dict) else {}
        if not guard and "api_call_allowed_now" not in payload and "blocked_by_checkpoint" not in payload:
            return
        lines.extend(
            [
                f"- api_call_allowed_now: {payload.get('api_call_allowed_now')}",
                f"- qualification_retry_allowed_now: {payload.get('qualification_retry_allowed_now')}",
                f"- qualification_retry_allowed_meaning: {payload.get('qualification_retry_allowed_meaning')}",
                f"- retry_preflight_clear_only: {payload.get('retry_preflight_clear_only')}",
                f"- retry_collection_authorized: {payload.get('retry_collection_authorized')}",
                f"- blocked_by_checkpoint: {payload.get('blocked_by_checkpoint')}",
                f"- checkpoint_path: {payload.get('checkpoint_path')}",
                f"- next_safe_action_status: {payload.get('next_safe_action_status')}",
            ]
        )
        if guard:
            lines.append(f"- api_guard_status: {guard.get('status')}")
            lines.append(
                "- api_guard_safety_violations: "
                + (", ".join(guard.get("safety_violations") or []) or "none")
            )
    ...
    if report.get("mode") == "applied":
        ...
        lines.extend(
            [
                ...
                f"- retry_candidate_unit_count: {before.get('retry_candidate_unit_count', before.get('retry_ready_unit_count'))}",
                f"- retry_ready_unit_count: {before.get('retry_ready_unit_count')}",
                f"- broad_retry_risk: {before.get('broad_retry_risk')}",
                f"- coverage_gap: {json.dumps(before.get('coverage_gap') or {}, ensure_ascii=False)}",
                f"- metadata_gaps: {json.dumps(before.get('metadata_gaps') or {}, ensure_ascii=False)}",
                "",
                "## After",
                "",
                f"- error_unit_count: {after.get('error_unit_count')}",
                f"- retry_candidate_unit_count: {after.get('retry_candidate_unit_count', after.get('retry_ready_unit_count'))}",
                f"- retry_ready_unit_count: {after.get('retry_ready_unit_count')}",
                f"- broad_retry_risk: {after.get('broad_retry_risk')}",
                f"- coverage_gap: {json.dumps(after.get('coverage_gap') or {}, ensure_ascii=False)}",
    ...
    lines = [
        ...
        f"- automatic_queue_execution_allowed: {report.get('automatic_queue_execution_allowed')}",
        ...
        f"- retry_candidate_unit_count: {report.get('retry_candidate_unit_count', report.get('retry_ready_unit_count'))}",
    ...
    coverage_gap = report.get("coverage_gap") or {}
    for key in (
        "collection_coverage",
        "release_target_ratio",
        "total_unit_count",
        "attempted_unit_count",
        "unattempted_unit_count",
        "target_attempted_unit_count",
        "additional_attempted_units_needed",
    ):
        lines.append(f"- {key}: {coverage_gap.get(key)}")
    lines.extend(
        [
            "",
            "## Major Coverage Gaps",
    ...
    for item in (report.get("major_coverage_gaps") or [])[:20]:
        lines.append(
            f"- {item.get('major_code') or ''} {item.get('major_name') or ''}: "
            f"coverage={item.get('coverage')}, unattempted={item.get('unattempted_unit_count')}, "
            f"errors={item.get('error_unit_count')}"
        )
    lines.extend([
        "",
        "## Metadata Gaps",
