...
def _render_ncs_discover_tools_markdown(result: dict[str, Any]) -> str | None:
    route = result.get("query_route")
    if not isinstance(route, dict):
        return None
    lines = ["## NCS 도구 라우팅"]
    intent = _clean_markdown_text(result.get("intent"))
    if intent:
        lines.append(f"- 질의: {intent}")
    lines.extend(
        [
            f"- 추천 도구: `{route.get('tool') or '-'}`",
            f"- 시나리오: `{route.get('scenario') or '-'}`",
            f"- 신뢰도: `{route.get('confidence')}`",
        ]
    )
    missing = route.get("missing_params")
    if isinstance(missing, list) and missing:
        lines.append(
            "- 추가 입력 필요: "
            + ", ".join(f"`{_clean_markdown_text(item)}`" for item in missing)
        )
    chain = route.get("expected_tool_chain")
    if isinstance(chain, list) and chain:
        lines.append(
            "- 권장 호출 순서: "
            + " → ".join(f"`{_clean_markdown_text(item)}`" for item in chain[:4])
        )
    return "\n".join(lines)


_PUBLIC_MARKDOWN_RENDERERS["ncs_discover_tools"] = _render_ncs_discover_tools_markdown
...
_configure_search_runtime(
    ...
    clamp_limit=lambda *args, **kwargs: clamp_limit(*args, **kwargs),
    unit_path=lambda *args, **kwargs: unit_path(*args, **kwargs),
    tier_predicates=lambda *args, **kwargs: _ncs_search_tier_predicates(*args, **kwargs),
    tier_executor=lambda *args, **kwargs: _execute_ncs_search_tiers(*args, **kwargs),
    token_expander=lambda *args, **kwargs: _validated_ncs_search_token_expansions(
        *args, **kwargs
    ),
)
