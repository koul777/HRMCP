...
class ApiResumeTests(unittest.TestCase):
    def setUp(self):
        ...
        self.options = dict(apply=True, sources=["job-base"], credentials={"job-base": "secret"}, output_path=self.root / "working.db", checkpoint_dir=self.root / "checkpoint")
    ...
    def test_resume_skips_committed_major_and_does_not_recopy(self):
        first = refresh_ncs_api_evidence(self.db, **self.options, callables=RefreshCallables(collect_job_base=self.collector(fail=True)))
        ...
        with patch("ncs_mcp.api_refresh_builder._prepare_working_copy", side_effect=AssertionError("must not copy")):
            final = refresh_ncs_api_evidence(self.db, **self.options, resume=True, callables=RefreshCallables(collect_job_base=resumed))
        self.assertEqual(final["outcome"], "succeeded_append_only", final)
        self.assertEqual(calls, ["02"])
        self.assertNotIn("secret", (self.root / "checkpoint/api_checkpoint.json").read_text())

    def test_source_change_refuses_resume(self):
        refresh_ncs_api_evidence(self.db, **self.options, callables=RefreshCallables(collect_job_base=self.collector(fail=True)))
        with closing(sqlite3.connect(self.db)) as conn, conn:
            conn.execute("UPDATE ksa_items SET ksa_text_raw='changed'")
        result = refresh_ncs_api_evidence(self.db, **self.options, resume=True)
    ...
    def test_source_parameter_change_refuses_resume(self):
        ...
        result = refresh_ncs_api_evidence(self.db, **options, resume=True)
        self.assertEqual(result["preflight_errors"], ["checkpoint_identity_mismatch"])
